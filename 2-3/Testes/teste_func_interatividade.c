...
AVR_23 *povoar_pessoas()
{
    ...
    for (int i = 0; i < quantidade_pessoas; i++)
    {
        ...
        if (i < 5)
        {
            ...
            cep_natal = strdup("00000-001"); // Exemplo de CEP natal
            cep_atual = strdup("00000-002"); // Exemplo de CEP atual
...
void testar_remocoes(LISTA_DUPLAMENTE *lista_estados, AVR_23 **raiz_pessoas)
{
    ...
    if (lista_estados != NULL)
    {
        ...
        CIDADE *cidade = procurar_cidade_por_CEP(lista_estados, cep_remover);
        if (cidade != NULL)
        {
            ...
            if (resultado)
            {
                ...
                mensagem_sucesso(buffer);
                free(cep_removido); // Liberar memória do CEP removido
            ...
            else
            {
                ...
                printf("CEP removido: %s (comportamento inesperado)\n", cep_removido);
            }
        }
    }
